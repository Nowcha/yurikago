...
- ダッシュボード「今週やること」の担当別セクション分け（現状は担当バッジ表示のみ）
- カテゴリ別進捗バー（現状は全体進捗のみ）
- 準備品のneededBy期限接近アラート（現状は予算集計のみ）
- 準備品への担当割当UI（型は対応済み）
- Phase 2「産後2週間スプリント」専用ビュー
- weekly-info.json の内容レビュー（ドラフト生成済み、母子手帳・産院資料との突合が必要）

解消済み: Service Worker（stale-while-revalidate方式で実装）、Phase 1.5週次サマリ（4〜41週・ダッシュボード表示込み）
